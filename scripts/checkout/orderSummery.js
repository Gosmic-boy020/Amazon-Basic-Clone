...
let cartSummaryHTML=``;
      ...
      <div class="cart-item-details-grid">
        ...
        <div class="cart-item-details">
          ...
          <div class="product-quantity">
            ...
            <input class="quantity-input js-quantity-input-${matchingProduct.id}" placeholder="1-10" >
            <span class="save-quantity-link link-primary" id="save" data-product-id="${matchingProduct.id}">Save</span>
  ...
  function deliveryOptionsHTML(matchingProduct,cartItem){
    ...
    deliveryOptions.forEach((delivaryOption) => {
      ...
      const dateString=deliveryDate.format('dddd,MMMM D');      
      const priceString = delivaryOption.priceCents === 0?'FREE':`$${formateCurrency(delivaryOption.priceCents)} - `;
      const isChecked=delivaryOption.id === cartItem.deliveryOptionId
      html+=`
        <div class="delivery-option js-delivery-option"
        data-product-id="${matchingProduct.id}"
        data-delivery-option-id="${delivaryOption.id}">
              <input type="radio"
              ${isChecked ? 'checked':''}
...
document.querySelector('.js-order-summary').innerHTML=cartSummaryHTML;
 ...
 function checkQuntity() {
  ...
  cart.forEach((cartItem)=> {
    checkOutCartQuantity+=cartItem.quantity;
  })
 document.querySelector('.return-to-home-link').innerHTML=checkOutCartQuantity+' items';
 }
checkQuntity()
